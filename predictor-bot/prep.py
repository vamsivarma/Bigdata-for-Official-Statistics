...
for Tesla company (TSLA) from yahoo finances portal:
...
def get_raw_xy(data):
    ...
    # adj close.
    # change - data=data.drop(columns=['Date','Adj Close'])
...
def get_vpo(values):
    ...
    Bur, it's shifting values one index backwards.
    ...
    X:m11,m12,m13  Y:m23    since we don't have any X data from Day 0
    X:m21,m22,m23  Y:m33
    X:m31,m32,m32  Y:No data <- We need to discard this as well,
